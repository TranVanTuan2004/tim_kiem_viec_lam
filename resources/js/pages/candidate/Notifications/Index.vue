...
import { Bell, BellOff, Trash2, Check, Briefcase, MapPin, DollarSign, TrendingUp, Calendar } from 'lucide-vue-next';
...
const markAsRead = (id: number) => {
    router.post(`/candidate/notifications/${id}/read`, {}, {
        preserveScroll: true,
    });
};
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
                    ...
                    <Card class="border-purple-200 bg-purple-50 transition-shadow hover:shadow-md">
                        <CardContent class="p-4 flex items-center justify-center">
                            <Button
                                v-if="unreadCount > 0"
                                @click="markAllAsRead"
                                variant="ghost"
                                size="sm"
                                class="text-purple-700 hover:text-purple-800 hover:bg-purple-100"
                            >
                                <Check class="w-4 h-4 mr-2" />
                                Đánh dấu tất cả
                            </Button>
                            <div v-else class="text-sm text-purple-700">Hoàn thành</div>
                        </CardContent>
                    </Card>
                </div>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle class="text-lg">Danh sách thông báo</CardTitle>
                    </CardHeader>
                    <CardContent class="p-0">
                        ...
                        <div v-if="notifications.data.length === 0" class="p-12 text-center">
                            ...
                            <p class="mt-2 text-sm text-gray-500">
                                Bạn sẽ nhận được thông báo khi có công việc phù hợp
                            </p>
                            <div class="mt-6">
                                <Button as-child>
                            ...
                            >
                                <div class="flex gap-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-start justify-between gap-4 mb-2">
                                            <div class="flex-1">
                                                <div class="flex items-center gap-2 mb-1">
                                                    <Badge :variant="notification.type === 'job_match' ? 'default' : 'secondary'">
                                        ...
                                        <div v-if="notification.type === 'job_match' && notification.data" class="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-100">
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                                                <div class="flex items-center gap-2 text-sm">
                                                    <Briefcase class="w-4 h-4 text-blue-600" />
                                                    <div>
                                                        <p class="text-xs text-gray-500">Công ty</p>
                                                        <p class="font-medium text-gray-900">{{ notification.data.company_name }}</p>
                                                    </div>
                                                </div>
                                                <div class="flex items-center gap-2 text-sm">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p class="font-medium text-gray-900">{{ notification.data.location }}</p>
                                                    </div>
                                                </div>
                                                <div class="flex items-center gap-2 text-sm">
                                                    <DollarSign class="w-4 h-4 text-emerald-600" />
                                                    <div>
                                                        <p class="text-xs text-gray-500">Mức lương</p>
                                                        <p class="font-medium text-gray-900">{{ notification.data.salary_range }}</p>
                                                    </div>
                                                </div>
                                                <div class="flex items-center gap-2 text-sm">
                                                    <TrendingUp class="w-4 h-4 text-blue-600" />
                                                    <div>
                                                        <p class="text-xs text-gray-500">Độ phù hợp</p>
                                                        <p class="font-bold text-blue-600">{{ notification.data.match_score }}%</p>
                                                    </div>
                                                </div>
                                            </div>
                                            <Link
                                                v-if="notification.data.job_posting_id"
                                        ...
                                        <div class="flex items-center gap-2 mt-4 pt-4 border-t border-gray-100">
                                            ...
                                            <Button
                                                ...
                                                :variant="deleteConfirm === notification.id ? 'destructive' : 'outline'"
                                                size="sm"
                                            >
                                                <Trash2 class="w-3.5 h-3.5 mr-1" />
                                                {{ deleteConfirm === notification.id ? 'Xác nhận xóa' : 'Xóa' }}
                                            </Button>
                                        </div>
                                    </div>
                                </div>
