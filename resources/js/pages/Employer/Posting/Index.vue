...
// === 3. Xóa tin tuyển dụng với toast và cập nhật trực tiếp ===
const deleteJob = (id: number) => {
    ...
    form.delete(`/employer/posting/${id}`, {
        onSuccess: () => {
            const index = props.jobs.data.findIndex(job => job.id === id);
            if (index !== -1) props.jobs.data.splice(index, 1);

            toast.success('✅ Xóa tin tuyển dụng thành công!', { position: 'top-right', autoClose: 2500 });
        },
        onError: () => {
            toast.error('❌ Xóa tin tuyển dụng thất bại.', { position: 'top-right', autoClose: 2500 });
        },
    });
};

// === 4. Ẩn / Hiện tin tuyển dụng với toast ===
const toggleJob = (job: any) => {
    ...
    form.patch(`/employer/posting/${job.id}/toggle`, {
        onSuccess: () => {
            ...
            toast.success(
                job.is_active
                    ? '✅ Tin tuyển dụng đã được hiển thị'
                    : '✅ Tin tuyển dụng đã bị ẩn',
                { position: 'top-right', autoClose: 2500 }
            );
        },
        onError: () => {
            toast.error('❌ Thao tác thất bại.', { position: 'top-right', autoClose: 2500 });
        },
    });
};
</script>
    ...
    <AppLayout>
        <div class="space-y-6 p-6">
            ...
            <div class="flex items-center justify-between">
                ...
                <Link href="/employer/posting/create">
                    <Button class="flex items-center gap-2"><Plus class="h-4 w-4" /> Đăng tin mới</Button>
                </Link>
            </div>
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex space-x-2 overflow-x-auto">
                ...
                >
                    <Button
                        :variant="filter.value === props.filters.status ? 'default' : 'outline'"
        ...
        <div v-else class="py-10 text-center text-gray-500">
            ...
            <Link href="/employer/posting/create" class="text-blue-600 underline">Đăng tin ngay</Link>
        </div>
        ...
        <Dialog :open="showRejectionModal" @update:open="showRejectionModal = $event">
            <DialogContent class="sm:max-w-[600px]">
                ...
                <div v-if="selectedRejectedJob" class="space-y-4 py-4">
                    ...
                    <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded">
                        <h4 class="font-semibold text-red-800 mb-2 flex items-center gap-2">
                            <AlertCircle class="h-4 w-4" /> Lý do từ chối:
                        </h4>
                ...
                <DialogFooter class="sm:justify-center gap-2">
                    <Button variant="outline" @click="showRejectionModal = false">Đóng</Button>
                    <Link v-if="selectedRejectedJob" :href="`/employer/posting/${selectedRejectedJob.id}/edit`">
                        <Button class="bg-blue-600 hover:bg-blue-700">Chỉnh sửa ngay</Button>
                    </Link>
                </DialogFooter>
            </DialogContent>
        </Dialog>
