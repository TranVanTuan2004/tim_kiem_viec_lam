...
async function sendMessage() {
    ...
    try {
        const response = await axios.post(
            `/admin/chat/send/${selectedUser.value.id}`,
            {
...
onMounted(() => {
    if (currentUserId.value && window.Echo) {
        ...
        channel.listen('.message.sent', (e: any) => {
            ...
            console.log(
                '💬 [Admin Chat] Current User ID:',
                currentUserId.value,
            );
            console.log(
                '💬 [Admin Chat] Selected User ID:',
            ...
            if (e.sender_id !== currentUserId.value) {
                ...
                if (
                    selectedUser.value &&
                    e.sender_id === selectedUser.value.id
                ) {
                    console.log(
                        '✅ [Admin Chat] Adding to current conversation',
                    );
                    ...
                    console.log(
                        '📬 [Admin Chat] Updating unread count for user',
                        e.sender_id,
                    );
                ...
                console.log(
                    '⏭️ [Admin Chat] Message from self, already added locally',
                );
            }
        });
        ...
        channel.subscribed(() => {
            console.log(
                '✅ [Admin Chat] Successfully subscribed to',
                channelName,
            );
        });
                ...
                <CardContent class="h-full p-0">
                    <div class="flex h-full">
                        ...
                        <div class="flex flex-1 flex-col">
                            ...
                            <!-- Message Input -->
                            <div v-if="selectedUser" class="border-t p-4">
                                <form
