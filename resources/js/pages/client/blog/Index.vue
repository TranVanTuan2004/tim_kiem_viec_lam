...
const perPage = ref((props.filters as any).per_page ?? (props.blogs as any).per_page ?? 6);
...
const paginate = (page: number) => {
    ...
    router.get('/blog', params, { preserveState: true, preserveScroll: true });
};

const visitLink = (link: any) => {
    if (!link?.url) return;
    try {
...
const visiblePages = computed(() => {
    const total = (props.blogs as any).last_page || 1;
    const current = (props.blogs as any).current_page || 1;
    const windowSize = 2;
        ...
        <div class="relative bg-gradient-to-br from-primary/20 via-primary/10 to-background py-20 md:py-24 overflow-hidden">
            ...
            <div class="container mx-auto px-4 relative z-10">
                <div class="max-w-4xl mx-auto text-center">
                    <div class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-primary/30 to-primary/10 backdrop-blur-md border border-primary/20 text-primary mb-8 shadow-xl hover:shadow-2xl transition-all duration-300">
                        <Sparkles class="h-4 w-4 animate-pulse" />
                        <span class="text-sm font-semibold">Blog IT - Nơi chia sẻ kiến thức</span>
                    </div>
                    <h1 class="text-6xl md:text-7xl lg:text-8xl font-extrabold mb-6 bg-gradient-to-r from-primary via-primary/80 to-primary/60 bg-clip-text text-transparent leading-tight tracking-tighter">
                        Blog IT
                    </h1>
        ...
        <div class="container mx-auto px-4 py-12">
            ...
            <div v-else class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="lg:col-span-8">
                    <div class="mb-4 flex items-center justify-between">
                        <div class="text-sm text-muted-foreground">
                            Hiển thị {{ (props.blogs as any).from || 0 }} - {{ (props.blogs as any).to || props.blogs.data.length }} trong {{ (props.blogs as any).total || props.blogs.data.length }} bài
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="text-sm text-muted-foreground">Bài/trang</span>
                            <select
                    ...
                    <div class="space-y-8">
                        <Link
                            ...
                            class="group block"
                        >
                            <div class="rounded-2xl p-[1.5px] bg-gradient-to-br from-primary/30 via-primary/10 to-transparent hover:from-primary/50 hover:via-primary/20 transition-all duration-300">
                                <Card class="rounded-2xl overflow-hidden border border-white/10 bg-background/95 backdrop-blur-sm shadow-xl transition-all duration-300 group-hover:shadow-2xl">
                                    <div class="flex flex-col md:flex-row">
                                        <div class="md:w-80 h-64 md:h-80 relative overflow-hidden bg-muted flex-shrink-0">
                                            ...
                                            <Badge
                                                ...
                                                class="absolute top-4 left-4 bg-gradient-to-r from-yellow-400 via-orange-500 to-red-500 text-white border-0 shadow-md z-10"
                                            >
                                        ...
                                        <div class="flex-1 p-6 md:p-8 flex flex-col">
                                            ...
                                            <h2 class="text-2xl md:text-3xl font-extrabold mb-3 group-hover:text-primary transition-colors line-clamp-2 leading-snug tracking-tight">
                                                {{ blog.title }}
                                            </h2>
                                            ...
                                            <div class="flex items-center justify-between pt-4 border-t border-border/50 mt-auto">
                                                ...
                                                <div class="flex items-center gap-1.5 text-primary text-sm font-semibold group-hover:gap-2 transition-all">
                                                    <span>Đọc thêm</span>
                    ...
                    >
                        <Button
                            ...
                            variant="outline"
                            size="sm"
                            @click="paginate(blogs.current_page - 1)"
                            class="gap-1"
                        >
                        ...
                        <template v-for="p in visiblePages" :key="`p-${p}`">
                            <Button
                                ...
                                :variant="p === blogs.current_page ? 'default' : 'outline'"
                                size="sm"
                                class="min-w-9"
                                @click="paginate(p)"
                            >
                ...
                <div class="lg:col-span-4 space-y-6">
                    <!-- Featured Blogs -->
                    <Card v-if="featuredBlogs.length > 0" class="border border-white/10 shadow-xl">
                        ...
                        <CardContent class="space-y-4 pt-2">
                            <Link
                                ...
                                class="group block p-4 rounded-xl border border-white/10 hover:border-primary/30 hover:shadow-lg transition-all bg-background/80 backdrop-blur-sm"
                            >
